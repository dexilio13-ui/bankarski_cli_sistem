...
class SQLiteKorisnikRepo(KorisnikRepo):
    def get_by_id(self, korisnik_id: UUID) -> Korisnik | None:
        with sqlite3.connect(DB_PATH) as conn:
            cur = conn.execute(
                "SELECT * FROM korisnici WHERE id=?", (str(korisnik_id),)
            )
            row = cur.fetchone()
...
class SQLiteRacunRepo(RacunRepo):
    ...
    def get_by_vlasnik(self, vlasnik_id: UUID) -> List[Racun]:
        with sqlite3.connect(DB_PATH) as conn:
            cur = conn.execute(
                "SELECT * FROM racuni WHERE vlasnik_id=?", (str(vlasnik_id),)
            )
            return [self._mapiraj_u_objekat(row) for row in cur.fetchall()]
    ...
    def _mapiraj_u_objekat(self, row: tuple) -> Racun:
        return Racun(
            id=UUID(row[0]),
            vlasnik_id=UUID(row[1]),
            tip=TipRacuna(row[2]),
            valuta=Valuta(row[3]),
            stanje=row[4],
            status=StatusRacuna(row[5]),
            kamatna_stopa=row[6],
            dozvoljeni_minus=row[7],
        )
...
class SQLiteTransakcijaRepo(TransakcijaRepo):
    def get_by_racun(self, racun_id: UUID) -> List[Transakcija]:
        with sqlite3.connect(DB_PATH) as conn:
            cur = conn.execute(
                "SELECT * FROM transakcije WHERE racun_id=?", (str(racun_id),)
            )
            return [self._mapiraj_u_objekat(row) for row in cur.fetchall()]
    ...
    def _mapiraj_u_objekat(self, row: tuple) -> Transakcija:
        t = Transakcija(
            id=UUID(row[0]), racun_id=UUID(row[1]), iznos=row[2], tip=row[3]
        )
        ...
        if cursor.fetchone()[0] == 0:
            ...
            racuni = [
                ...
                (
                    str(uuid4()),
                    id_pera,
                    TipRacuna.TEKUCI.value,
                    Valuta.RSD.value,
                    50000.0,
                    StatusRacuna.AKTIVAN.value,
                    None,
                    None,
                ),
                (
